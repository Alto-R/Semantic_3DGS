...
from scripts.task1.dense_seg.ade20k_ontology import ProjectOntology, load_ontology
from scripts.task1.dense_seg.dense_seg_backends import BACKEND_NAMES, build_backend
...
def class_color(class_name: str) -> tuple[int, int, int]:
    # rgb_for_class falls back to a stable digest-based color for classes
    # outside the curated palette, so colors stay deterministic across runs.
    return tuple(int(round(value * 255.0)) for value in rgb_for_class(class_name))
