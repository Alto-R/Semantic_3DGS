#!/usr/bin/env python3
"""End-to-end DINOv3 abstention-recovery materialization for one scene.

This pipeline runs every consensus/recovery/materialization stage in one
invocation from a complete dense DINOv3 FlashSplat vote manifest and writes a
review materialization:

1. load the DINOv3 per-view FlashSplat votes (one normalized distribution per
   camera/Gaussian);
...
No DINOv3 inference or vote lifting is rerun; the caller supplies the vote
manifest produced from the scene's real cameras.  The output is a review
artifact for visual comparison; it is not auto-accepted.
"""
...
SOURCE = "dinov3_abstention_recovery_materialization"
CONTRACT = "review_dinov3_abstention_recovery_materialization_v1"
TARGET_STATUSES = (
    STATUS_SINGLE_CAMERA,
    STATUS_EXACT_TIE,
    STATUS_NO_STRICT_MAJORITY,
...
def load_dinov3_votes(
    ...
    *,
    ...
    validate_vote_manifest(manifest, name="DINOv3")
    ...
    for frame in manifest["frames"]:
        ...
        with np.load(vote_path, allow_pickle=False) as data:
            winners, mass, _summary = collapse_camera_with_mass(
                data["indices"],
                data["class_ids"],
                data["weights"],
        ...
        if supported.size:
            # One unique winner per camera/Gaussian, so direct indexing is
            # equivalent to bincount and cannot collide within this camera.
            counts[
                winners[supported].astype(np.int64, copy=False), supported
            ] += np.uint16(1)
        evidence.append(
            {
...
def main() -> None:
    ...
    gaussian_count = read_vertex_count(args.source_ply)

    print("loading DINOv3 votes...", flush=True)
    ...
    if np.any(
        (source_codes >= 2)
        & ~np.isin(status, np.asarray(TARGET_STATUSES, dtype=np.uint8))
    ):
        raise ValueError("recovery labels were assigned outside detected abstentions")
    if np.any((source_codes == 0) & (labels != 0)):
