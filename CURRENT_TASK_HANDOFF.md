...
Storage policy: `semantic_point_cloud_rgb_debug.ply`, its metadata JSON, and its
inspection JSON are redundant with the SuperSplat debug export and are no
longer generated by the scene pipeline. Existing copies may be deleted from all
runs. Preserve each accepted run's deliverables, fusion arrays/summaries,
SuperSplat debug PLYs, overlays, contact sheets, validation reports, and logs.
