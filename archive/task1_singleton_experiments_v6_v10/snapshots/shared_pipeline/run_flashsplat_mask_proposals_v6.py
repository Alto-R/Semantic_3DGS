...
def frame_mask_path(
    ...
    """Resolve an optional manifest-relative augmented mask stack."""
    configured = str(frame.get("mask_path", "")).strip()
    if configured:
        path = Path(configured)
        return path if path.is_absolute() else manifest_path.parent / path
    return mask_dir / str(frame["mask_file"])
...
def main() -> None:
    ...
    with torch.no_grad():
        for frame in sam_manifest["frames"]:
            ...
            mask_path = frame_mask_path(manifest_path, mask_dir, frame)
