...
CROSS_VIEW_VERIFICATION_SOURCE = "cross_view_sam_verification"
...
class SemanticGroup:
    ...
    independent_source_frames: set[str] = field(default_factory=set)
    verification_source_frames: set[str] = field(default_factory=set)
    recovery_seed_keys: set[str] = field(default_factory=set)
...
def add_proposal_to_group(group: SemanticGroup, proposal: SemanticProposal) -> None:
    ...
    group.source_frames.add(frame_file)
    if str(proposal.metadata.get("source", "")) == CROSS_VIEW_VERIFICATION_SOURCE:
        group.verification_source_frames.add(frame_file)
        seed_key = str(proposal.metadata.get("cross_view_seed_key", "")).strip()
        if seed_key:
            group.recovery_seed_keys.add(seed_key)
    else:
        group.independent_source_frames.add(frame_file)
...
def merge_stuff_groups(groups: list[SemanticGroup], stuff_classes: set[str]) -> list[SemanticGroup]:
    ...
    for group in groups:
        ...
        target.independent_source_frames.update(group.independent_source_frames)
        target.verification_source_frames.update(group.verification_source_frames)
        target.recovery_seed_keys.update(group.recovery_seed_keys)
...
def filter_groups(
    ...
    min_recovery_verification_views: int = 2,
) -> list[SemanticGroup]:
    def recovery_evidence_is_valid(group: SemanticGroup) -> bool:
        if not group.verification_source_frames:
            return group.proposal_count >= min_group_proposals
        if len(group.independent_source_frames) >= min_group_proposals:
            return True
        return (
            len(group.independent_source_frames) >= 1
            and len(group.verification_source_frames)
            >= min_recovery_verification_views
        )
    ...
    kept = [
        ...
        and recovery_evidence_is_valid(group)
    ]
...
def merged_component_group(
    ...
    for group in source_groups:
        ...
        merged.independent_source_frames.update(group.independent_source_frames)
        merged.verification_source_frames.update(group.verification_source_frames)
        merged.recovery_seed_keys.update(group.recovery_seed_keys)
        ...
        pruned.append(
            {
                ...
                "independent_source_view_count": len(group.independent_source_frames),
                "verification_source_view_count": len(group.verification_source_frames),
                "recovery_seed_keys": sorted(group.recovery_seed_keys),
                "score": group.score,
...
def main() -> None:
    ...
    parser.add_argument("--min-recovery-verification-views", default=2, type=int)
    ...
    if args.min_recovery_verification_views < 2:
        raise ValueError("min-recovery-verification-views must be at least 2")
    ...
    groups = filter_groups(
        ...
        args.min_recovery_verification_views,
    )
