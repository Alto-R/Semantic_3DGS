...
def main() -> None:
    ...
    parser.add_argument(
        "--enable-experimental-cross-view-recovery",
        action="store_true",
        help=(
            "Enable the rejected v6 mask-augmentation experiment. Disabled by "
            "default so ordinary runs preserve the v5 proposal topology."
        ),
    )
    parser.add_argument("--skip-cross-view-recovery", action="store_true")
    parser.add_argument("--cross-view-min-seed-gaussians", default=1500, type=int)
    parser.add_argument("--cross-view-min-seed-sam-score", default=0.90, type=float)
    parser.add_argument("--cross-view-min-seed-mask-area-ratio", default=0.02, type=float)
    parser.add_argument(
        "--cross-view-max-nested-thing-containment",
        default=0.80,
        type=float,
    )
    parser.add_argument("--cross-view-max-target-views", default=4, type=int)
    parser.add_argument("--cross-view-min-verification-views", default=2, type=int)
    parser.add_argument(
        "--cross-view-min-verification-sam-score",
        default=0.90,
        type=float,
    )
    parser.add_argument("--cross-view-min-projection-coverage", default=0.60, type=float)
    parser.add_argument("--cross-view-max-seeds", default=24, type=int)
    ...
    experimental_cross_view = (
        args.enable_experimental_cross_view_recovery
        and not args.skip_cross_view_recovery
    )
    ...
    if not args.skip_proposal_generation:
        ...
        initial_proposal_log = (
            "grounding_02_flashsplat_seed.log"
            if experimental_cross_view
            else "grounding_02_flashsplat.log"
        )
        run_logged(command, args.log_dir / initial_proposal_log)

        if experimental_cross_view:
            recovery_command = [
                python,
                "scripts/task1/recover_cross_view_sam_masks.py",
                "--model-path",
                str(args.model_path),
                "--ground-output-dir",
                str(args.ground_output_dir),
                ...
                "30000",
                "--min-seed-gaussians",
                str(args.cross_view_min_seed_gaussians),
                "--min-seed-sam-score",
                str(args.cross_view_min_seed_sam_score),
                "--min-seed-mask-area-ratio",
                str(args.cross_view_min_seed_mask_area_ratio),
                "--max-nested-thing-containment",
                str(args.cross_view_max_nested_thing_containment),
                "--max-target-views",
                str(args.cross_view_max_target_views),
                "--min-verification-views",
                str(args.cross_view_min_verification_views),
                "--min-verification-sam-score",
                str(args.cross_view_min_verification_sam_score),
                "--min-projection-coverage",
                str(args.cross_view_min_projection_coverage),
                "--max-seeds",
                str(args.cross_view_max_seeds),
                "--max-masks-per-view",
                "0",
            ]
            run_logged(
                recovery_command,
                args.log_dir / "grounding_03_cross_view_sam.log",
            )

            manifest_value = command.index("--manifest-name") + 1
            command[manifest_value] = "grounded_sam_cross_view_manifest.json"
            max_masks_value = command.index("--max-masks-per-view") + 1
            command[max_masks_value] = "0"
            run_logged(
                command,
                args.log_dir / "grounding_04_flashsplat_augmented.log",
            )
    ...
    command = [
        ...
        "--min-group-proposals",
        "2",
        "--min-recovery-verification-views",
        str(args.cross_view_min_verification_views),
        "--merge-iou",
        "0.35",
