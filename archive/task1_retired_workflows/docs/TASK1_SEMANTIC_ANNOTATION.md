...
- EyeNavGS Rutgers and NTHU trace repos are cloned under the cluster `external/`
  directory.
- `data/EyeNavGS/Rutgers` and `data/EyeNavGS/NTHU` are symlinks to
  those cloned trace repos.
- Trace inventory can be checked with:

  ```bash
  python scripts/task1/inventory_eyenavgs_traces.py
  ```
...
- GraphDeco official pretrained models are downloaded with
  `scripts/setup/download_graphdeco_models.sh`.
- After extraction, discover available PLY files with:

  ```bash
  python scripts/task1/find_point_clouds.py data/3dgs_models/graphdeco
  ```

  Build the initial scene/model manifest with:

  ```bash
  python scripts/task1/build_task1_manifest.py
  ```

  Current GraphDeco model manifest:

  - 26 `point_cloud.ply` files extracted under
    `data/3dgs_models/graphdeco`.
  ...
  - Manifest JSON was saved to:
    `outputs/eyenavgs_task1/manifests/task1_scene_manifest_graphdeco.json`.
...
2. Inspect the PLY:
   ...
   ```bash
   python scripts/task1/inspect_ply.py data/3dgs_models/<scene>/point_cloud.ply --json
   ```
...
8. Export final scene artifacts:
   ...
   ```text
   outputs/eyenavgs_task1/<scene>/semantic_point_cloud.ply
   outputs/eyenavgs_task1/<scene>/label_map.json
   outputs/eyenavgs_task1/<scene>/overlay_renders/
   ```
...
```bash
python archive/task1_legacy/scripts/add_dummy_labels.py data/3dgs_models/<scene>/point_cloud.ply outputs/semantic_test.ply --label 0
python scripts/task1/inspect_ply.py outputs/semantic_test.ply --json
```
...
bash scripts/setup/build_graphdeco_rasterizer.sh
sbatch archive/task1_legacy/scripts/slurm_task1_bicycle_pilot.sbatch
```

Scheduler resources are environment-specific. Override them when submitting:
...
```text
outputs/eyenavgs_task1/bicycle/point_cloud_inspection.json
outputs/eyenavgs_task1/bicycle/semantic_point_cloud.ply
outputs/eyenavgs_task1/bicycle/semantic_point_cloud_inspection.json
outputs/eyenavgs_task1/bicycle/label_map.json
outputs/eyenavgs_task1/bicycle/rgb_renders/
```
...
`scripts/setup/build_graphdeco_rasterizer.sh`; this avoids using the
...
`scripts/task1/check_graphdeco_projection.py`:
...
```bash
sbatch --export=ALL,RENDER_MAX_WIDTH=640 archive/task1_legacy/scripts/slurm_task1_bicycle_pilot.sbatch
```

To return to evenly spaced camera sampling, submit with an empty camera list:

```bash
sbatch --export=ALL,RENDER_CAMERA_INDICES=,RENDER_COUNT=20 archive/task1_legacy/scripts/slurm_task1_bicycle_pilot.sbatch
```
...
```bash
# Run from the repository root.
sbatch scripts/slurm/slurm_task1_bicycle_grounded_sam.sbatch
```
...
```bash
# Run from the repository root.
sbatch --export=ALL,OUTPUT_NAME=bicycle_semantic_targeted_v1,AUTO_TARGET_VIEW_COUNT=20,TARGET_CANDIDATE_COUNT=100,TARGET_SELECTION_SOURCE=outputs/eyenavgs_task1/accepted/bicycle scripts/slurm/slurm_task1_bicycle_grounded_sam.sbatch
```
...
The targeted-camera run completed in 9m05s and passed structural validation.
...
ambiguity. This run is therefore accepted as the current bicycle baseline.

### Scene-Configurable Workflow And Train Pilot

`scripts/slurm/slurm_task1_semantic_scene.sbatch` generalizes the accepted pipeline
...
The initial 50-view train run completed structurally, but pruned its strongly
supported sky group because 8,451 assigned Gaussians fell below the default
10,000-Gaussian stuff threshold. The reuse run lowered that threshold to
8,000 and established the accepted baseline:

```text
outputs/eyenavgs_task1/accepted/train
  -> ../train_semantic_baseline_v2
```

The reuse run retains two train instances of 428,735 and 18,476 Gaussians and adds
...
The first targeted train run performed that expansion and added 20 low-coverage/pose-diverse
...
labeling rather than object-instance labeling. For that targeted run, the stuff rule
...
The corrected reuse run confirmed the automatic fusion outcome, with 8,943
...
The final train run inherited the complete manifest and validated all 70 semantic
...
The initial room run completed structurally, with 14 nonzero labels and an unlabeled
...
The corrected room baseline reran detection and FlashSplat and is accepted at:

```text
outputs/eyenavgs_task1/accepted/room
...
`0.6700577548576253` pooled and `0.07734897022925243` minimum for the diagnostic
initial run. Visual QA shows consistent piano/television localization and
...
The first targeted room run completed structurally with 70 views, 21 nonzero labels,
...
The `truck_semantic_baseline_v1` run completed in 4 minutes 8 seconds and
...
3,500 while the weak fragment remains subject to 4,800. The corrected reuse run
isolated that fusion change and established the valid 50-view baseline.
...
The corrected baseline measures `0.963010689091956` pooled visible-tint coverage with a
...
The `truck_semantic_targeted_v1` run completed in 7 minutes 10 seconds
and is accepted at:

```text
outputs/eyenavgs_task1/accepted/truck
...
SEMANTIC_CAMERA_INDICES='70,115,68' SEMANTIC_VIEW_COUNT=3 MAX_DETECTIONS_PER_VIEW=12 MASK_BATCH_SIZE=4 \
  ...
  scripts/slurm/slurm_task1_bicycle_grounded_sam.sbatch
```
...
- `archive/task1_legacy/scripts/export_debug_label_colors.py`
...
- `scripts/task1/export_supersplat_label_colors.py`
...
```bash
sbatch --export=ALL,OUTPUT_NAME=bicycle_semantic_identity_test,REUSE_SOURCE_OUT=outputs/eyenavgs_task1/bicycle_semantic \
  scripts/slurm/slurm_task1_bicycle_grounded_sam.sbatch
```
...
The signed-evidence bicycle baseline completed successfully:
...
Compared with the spatial-only run, the signed negative evidence removes
...
The first identity-test run demonstrated why accepted labels must remain atomic:
...
labels. That run is therefore diagnostic only. The corrected merge-only rule
uses connected components to union source IDs but never subdivides an accepted
source label.

The corrected identity-test run passed all acceptance checks and remains the
...
- 3,694,666 unlabeled Gaussians, exactly matching the signed-evidence baseline
- clean bicycle/bench overlays across the same 50 views
- structural validation status `ok`

The automatic targeted-camera run is the current accepted bicycle output:
...
The accepted output is exposed without copying large artifacts, while the
corrected identity-test directory remains unchanged:

```text
outputs/eyenavgs_task1/accepted/bicycle
...
For the corrected bicycle identity baseline, the report measured 28,162,913 changed pixels
...
For the accepted targeted run, the full 70-view report measures
...
```bash
# Run from the repository root.
sbatch archive/task1_legacy/scripts/slurm_task1_bicycle_auto_proposals.sbatch
```
...
AUTO_CAMERA_INDICES='70,115,68' AUTO_VIEW_COUNT=3 MAX_MASKS_PER_VIEW=8 MASK_BATCH_SIZE=4 \
  ...
  archive/task1_legacy/scripts/slurm_task1_bicycle_auto_proposals.sbatch
```

Active scripts:

- `archive/task1_legacy/scripts/generate_sam_auto_masks.py`
  - renders views from `cameras.json`
...
- `archive/task1_legacy/scripts/cluster_flashsplat_proposals.py`
...
```text
outputs/eyenavgs_task1/bicycle_auto/sam_auto/
outputs/eyenavgs_task1/bicycle_auto/flashsplat_proposals/
outputs/eyenavgs_task1/bicycle_auto/semantic_point_cloud_auto.ply
outputs/eyenavgs_task1/bicycle_auto/label_map_auto.json
outputs/eyenavgs_task1/bicycle_auto/auto_group_summary.json
outputs/eyenavgs_task1/bicycle_auto/sam_auto_contact_sheet.png
outputs/eyenavgs_task1/bicycle_auto/label_overlay_renders/
outputs/eyenavgs_task1/bicycle_auto/label_overlay_contact_sheet.png
```
...
```bash
python scripts/task1/create_label_review_template.py \
  --auto-label-map outputs/eyenavgs_task1/bicycle_auto/label_map_auto.json \
  --auto-summary outputs/eyenavgs_task1/bicycle_auto/auto_group_summary.json \
  --output outputs/eyenavgs_task1/bicycle_auto/label_review.csv
```
...
```bash
python scripts/task1/apply_label_review.py \
  --auto-labels outputs/eyenavgs_task1/bicycle_auto/gaussian_labels_auto.npy \
  --review-csv outputs/eyenavgs_task1/bicycle_auto/label_review.csv \
  --input-ply data/3dgs_models/graphdeco/bicycle/point_cloud/iteration_30000/point_cloud.ply \
  --output-dir outputs/eyenavgs_task1/bicycle \
  --scene bicycle \
  --overwrite
```
...
outputs/eyenavgs_task1/bicycle/semantic_point_cloud.ply
outputs/eyenavgs_task1/bicycle/label_map.json
outputs/eyenavgs_task1/bicycle/gaussian_labels.npy
outputs/eyenavgs_task1/bicycle/label_review_summary.json
```
...
python scripts/task1/validate_task1_outputs.py \
  --labels-npy outputs/eyenavgs_task1/bicycle/gaussian_labels.npy \
  --label-map outputs/eyenavgs_task1/bicycle/label_map.json \
  --semantic-ply outputs/eyenavgs_task1/bicycle/semantic_point_cloud.ply
```
