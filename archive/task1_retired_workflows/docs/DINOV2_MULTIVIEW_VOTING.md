...
guard label to enter the final merge. The cached `media_console` mask was
rejected and removed from the selectable extension candidates because it
leaked broadly into wall and floor. `guitar` remains an explicit-only no-op
candidate. Pass
`GROUNDING_EXTENSION_CLASSES` to run a single candidate during further QA.

Train QA showed that the railroad was partitioned between the cached
`railroad_track` and `railway_platform` source masks even though no distinct
platform is present. The reviewed Train extension config therefore declares a
generic source-class union that emits their combined mask as one
`railroad_track` output group. Future Train prompting places the former
platform phrases under the railroad-track class rather than exposing a
competing platform class. Stump stump/log candidates were similarly retired
after they fragmented ordinary tree geometry; those prompt phrases now belong
to the single `tree` class.

The five additional DINOv2 bases were reviewed against their original RGB
views before defining further custom vocabulary. Flowers needs no extension.
The first standalone source pass tested Bonsai `bonsai_tree` and
`electronic_keyboard`; Counter `oven_mitt`, `mixing_bowl`,
`water_filter_pitcher`, and `onion`; Garden `garden_statue`; and Kitchen
`toy_bulldozer`. Only Bonsai `bonsai_tree` and Counter `mixing_bowl` survived
class-level review. Keyboard absorbed the bonsai stand, one oven-mitt group was
not a mitt, pitcher included another container, onion produced no fused group,
garden statue absorbed the table, and toy bulldozer leaked across table and
placemat regions. Rejected classes are not merge defaults, and Garden's active
extension config was removed.

The custom-only v2 evidence pass uses explicit versioned configs. Bonsai tests
appearance-specific electronic-keyboard prompts; Counter tests a tighter
oven-mitt vocabulary plus `cutlery`; and Kitchen tests the tighter oven-mitt
vocabulary plus narrower bulldozer prompts that omit generic construction-
vehicle and front-loader phrases. All v2 merge defaults are empty pending
visual review. Food-like produce and snacks, Garden bench and pot, and Kitchen
chair are skipped because ADE20K/DINOv2 already contains `food`, `bench`, `pot`,
and `chair`. Tablecloth, rolling pin, egg carton, placemat, ambiguous garden
clutter, and broad surface corrections remain out of scope. Standalone source
fusion must be reviewed before any class is merged into an immutable DINOv2
base.
