...
class SchedulerContractTest(unittest.TestCase):
    def test_scheduler_requires_gate_and_rejects_manual_controls(self) -> None:
        source = SCHEDULER.read_text(encoding="utf-8")
        for expected in (
            "TWO_SCENE_GATE_PATH",
            "SOURCE_VALIDATION_OUTPUT_NAME",
            "Manual camera selection and consensus overrides are not accepted",
            "zero_camera_fill_used=0",
            "propagation_used=0",
            "materialize_abstention_recovery",
            "semantic_point_cloud_colored.ply",
        ):
