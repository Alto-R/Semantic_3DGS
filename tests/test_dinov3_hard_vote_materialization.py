...
class HardVoteMaterializationTest(unittest.TestCase):
    ...
    def test_scheduler_outputs_semantic_and_colored_ply_without_manual_selection(self) -> None:
        source = SCHEDULER.read_text(encoding="utf-8")
        self.assertIn("SOURCE_HARD_AUDIT_OUTPUT_NAME", source)
        self.assertIn("Manual CAMERA_INDICES and VIEW_COUNT are not accepted", source)
        self.assertIn("Consensus thresholds are fixed", source)
        self.assertIn("materialize_hard_vote_consensus", source)
        self.assertIn("semantic_point_cloud.ply", source)
        self.assertIn("semantic_point_cloud_colored.ply", source)
        self.assertIn("export_supersplat_label_colors", source)
